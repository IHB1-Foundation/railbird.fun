...
export default async function EvolutionPage() {
  ...
  const allEvents = timeline.agents
    .flatMap((a) =>
      a.strategies.map((s) => ({
        agent: a.agent,
        version: s.version,
  ...
  return (
    <section className="page-section">
      ...
      <div
        style={{ display: "flex", alignItems: "baseline", gap: "0.75rem", marginBottom: "1.25rem" }}
      >
      ...
      {!hasData ? (
        <div className="card">
          <p style={{ fontSize: "0.85rem", color: "var(--muted)", padding: "1rem" }}>
            No strategy evolution data yet. Agents update their strategy after every 20 hands. Check
            back in a few minutes once more hands have been played.
          </p>
        </div>
      ) : (
        <>
          ...
          <div className="card" style={{ marginBottom: "1rem", padding: "1rem 1.2rem" }}>
            ...
            <ErrorBoundary label="Strategy Timeline">
              <LazyStrategyTimeline agents={timeline.agents} metric="aggressionBps" height={160} />
            </ErrorBoundary>
          </div>

          <div className="card" style={{ marginBottom: "1rem", padding: "1rem 1.2rem" }}>
            <h3 className="section-title-sm" style={{ marginBottom: "0.5rem" }}>
              Strategy Timeline — Tightness
            </h3>
            <LazyStrategyTimeline agents={timeline.agents} metric="tightnessBps" height={140} />
          </div>
          ...
          >
                ...
                >
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    ...
                    <span style={{ fontWeight: 600 }}>
                      {(meta.averages.aggressionBps / 100).toFixed(0)}%
                    </span>
                  </div>
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    <span style={{ color: "var(--muted)" }}>Avg Tightness</span>
                    <span style={{ fontWeight: 600 }}>
                      {(meta.averages.tightnessBps / 100).toFixed(0)}%
                    </span>
                  </div>
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    <span style={{ color: "var(--muted)" }}>Avg Bluff Freq</span>
                    <span style={{ fontWeight: 600 }}>
                      {(meta.averages.bluffFreqBps / 100).toFixed(0)}%
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div className="card" style={{ padding: "1rem 1.2rem" }}>
              <h3 className="section-title-sm" style={{ marginBottom: "0.5rem" }}>
                <Tooltip text="ELO is a competitive rating system (starting at 1500) where beating stronger opponents earns more points. Higher ELO = better performance.">
                  ELO
                </Tooltip>{" "}
                vs Aggression
              </h3>
              ...
              <LazyEloStrategyScatter data={scatterData} width={280} height={160} />
            </div>
          </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
                {allEvents.map((e, i) => {
                  ...
                  return (
                    ...
                    >
                      <span
                        style={{ color: "var(--muted)", whiteSpace: "nowrap", minWidth: "32px" }}
                      >
                        v{e.version}
                      </span>
                      <span
                        style={{ fontWeight: 600, color: profile?.accentColor ?? "var(--text)" }}
                      >
                        {name}
                      </span>
                      <span style={{ color: "var(--muted)" }}>·</span>
                      <span>
                        A:{(e.aggressionBps / 100).toFixed(0)}% T:
                        {(e.tightnessBps / 100).toFixed(0)}% B:{(e.bluffFreqBps / 100).toFixed(0)}%
                      </span>
                      <span
                        style={{ marginLeft: "auto", color: "var(--muted)", fontSize: "0.68rem" }}
                      >
